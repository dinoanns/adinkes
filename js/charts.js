...
const percentageLabel = (context) => `${context.dataset.label}: ${context.parsed.y}%`;
...
const _ht = (typeof window.htChartData !== 'undefined') ? window.htChartData : null;
...
if (bpControlledCanvas) { createChart(bpControlledCanvas, bpControlledConfig); }
...
if (bpUncontrolledCanvas) { createChart(bpUncontrolledCanvas, bpUncontrolledConfig); }
...
if (ltfu3MonthCanvas) { createChart(ltfu3MonthCanvas, ltfu3MonthConfig); }
...
registrationsConfig.options.scales.yMonthlyRegistrations = {
  display: false, beginAtZero: true, max: _maxBaru,
};
registrationsConfig.options.plugins.tooltip.displayColors = true;
registrationsConfig.options.plugins.tooltip.callbacks = {
  labelColor: (ctx) => ({ borderColor: "#fff", backgroundColor: ctx.dataset.borderColor, borderWidth: 1 }),
};
const registrationsCanvas = document.getElementById("registrations");
if (registrationsCanvas) { createChart(registrationsCanvas, registrationsConfig); }
...
if (ltfu12MonthsCanvas) { createChart(ltfu12MonthsCanvas, ltfu12MonthsConfig); }
...
var _maxDmReg  = (_dmD.terdaftar && _dmD.terdaftar.length) ? Math.max(..._dmD.terdaftar, 1) : 1;
var _maxDmBaru = (_dmD.baru      && _dmD.baru.length)      ? Math.max(..._dmD.baru, 1)      : 1;
var dmRegistrationsConfig = baseLineChartConfig();
dmRegistrationsConfig.data = {
  labels: _dmL,
  datasets: [
    { label: "Registrasi kumulatif",   data: _dmD.terdaftar || [], borderColor: "#007ee4", backgroundColor: "transparent", yAxisID: "y" },
    { label: "Pasien dalam perawatan", data: _dmD.dalam_pwr || [], borderColor: "#b51bdc", backgroundColor: "transparent", yAxisID: "y" },
    { type: "bar", label: "Registrasi bulanan", data: _dmD.baru || [], borderColor: "#BEDFF9", backgroundColor: "#BEDFF9", yAxisID: "yMonthlyRegistrations" },
  ],
};
...
dmRegistrationsConfig.options.scales.yMonthlyRegistrations = { display: false, beginAtZero: true, max: _maxDmBaru };
dmRegistrationsConfig.options.plugins.tooltip.displayColors = true;
dmRegistrationsConfig.options.plugins.tooltip.callbacks = { labelColor: (ctx) => ({ borderColor: "#fff", backgroundColor: ctx.dataset.borderColor, borderWidth: 1 }) };
var dmRegistrationsCanvas = document.getElementById("dmregistrations");
if (dmRegistrationsCanvas) createChart(dmRegistrationsCanvas, dmRegistrationsConfig);
  ...
  datasets: [
    {
      ...
      backgroundColor: "transparent",
      yAxisID: "y",
      segment: { borderDash: (ctx) => dynamicChartSegementDashed(ctx, _dmN3v) },
    },
    {
      type: "bar",
...
if (dmUncontrolledCanvas) createChart(dmUncontrolledCanvas, dmUncontrolledConfig);

// dmltfu12months — hilang tindak lanjut 12 bulan
var dmLtfu12Config = baseLineChartConfig();
dmLtfu12Config.data = {
  labels: _dmL,
  datasets: [{ label: "Hilang tindak lanjut 12 bulan", data: _dmD.ltfu12 || [], borderColor: "#FF3355", backgroundColor: "rgba(255,51,85,0.1)", segment: { borderDash: (ctx) => dynamicChartSegementDashed(ctx, _dmNv) } }],
};
...
dmControlledConfig.data = {
  labels: _dmL3,
  datasets: [{ label: "Gula darah terkontrol", data: _dmD.dm_ok ? _dmD.dm_ok.slice(-3) : [], borderColor: "#3BB231", backgroundColor: "rgba(69,205,57,0.1)", segment: { borderDash: (ctx) => dynamicChartSegementDashed(ctx, _dmN3v) } }],
};
dmControlledConfig.options.plugins.tooltip.callbacks = { label: percentageLabel };
...
dmLtfu3Config.data = {
  labels: _dmL3,
  datasets: [{ label: "Tidak berkunjung 3 bulan", data: _dmD.ltfu3 ? _dmD.ltfu3.slice(-3) : [], borderColor: "#ed6300", backgroundColor: "rgba(230,137,70,0.1)", segment: { borderDash: (ctx) => dynamicChartSegementDashed(ctx, _dmN3v) } }],
};
...
dmStatinTrendConfig.data = {
  labels: _dmL,
  datasets: [{ label: "Statin diresepkan", data: _dmD.statin || [], borderColor: "#34aea0", backgroundColor: "rgba(52,174,160,0.1)", segment: { borderDash: (ctx) => dynamicChartSegementDashed(ctx, _dmNv) } }],
};
dmStatinTrendConfig.options.plugins.tooltip.callbacks = { label: percentageLabel };
dmStatinTrendConfig.options.scales.y.ticks.callback = (val) => val + "%";
var dmStatinTrendCanvas = document.getElementById("dmstatins");
if (dmStatinTrendCanvas) createChart(dmStatinTrendCanvas, dmStatinTrendConfig);
    ...
    datasets: [
      {
        ...
        borderColor: "#3BB231",
        backgroundColor: "rgba(69,205,57,0.1)",
        segment: { borderDash: (ctx) => dynamicChartSegementDashed(ctx, _dmNv) },
      },
      {
        label: "TD <130/80",
        data: _dmD.bp_ok130,
...
if (dmscreeningsCanvas && _dmD.skrining) {
  createChart(dmscreeningsCanvas, {
    ...
    options: {
      ...
      scales: {
        yPct:   { type: "linear", position: "right", min: 0, max: 100, ticks: { callback: (v) => v + "%" }, grid: { drawOnChartArea: false } },
        yCount: { type: "linear", position: "left",  min: 0, ticks: { display: false }, grid: { drawTicks: false } },
...
const _htP = (typeof window.htChartData !== 'undefined') ? window.htChartData : null;
...
patientsProtectedConfig.options.scales.y.max = _maxProtected + Math.round(_maxProtected * 0.1);
